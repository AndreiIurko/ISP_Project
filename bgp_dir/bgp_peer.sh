...
ip a add 10.202.0.1/17 dev ens3
ip a add fc00:2002::1/97 dev ens3
...
protocol static ipv4_routes {		
	ipv4;
	route 10.202.0.2/32 via 10.202.0.1;
	route 10.202.0.3/32 via 10.202.0.1;
	route 10.202.128.0/17 via 10.202.0.3;
}

protocol static ipv6_routes {
	ipv6;
	route fc00:2002::2/128 via fc00:2002::1;
	route fc00:2002::3/128 via fc00:2002::1;
	route fc00:2002::8000:0/97 via fc00:2002::3;
}

filter rt_import_4
{
	if (net = 10.202.0.0/16) then
	{
		reject;
	}
